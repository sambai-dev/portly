...
| 5 | TOML config + themes, zero-config default preserved | Homedash YAML w/ unknown-field rejection; statui layered TOML; corsa themes | system config dir per platform (`~/.config` on Linux, `~/Library/Application Support` on macOS, `%APPDATA%` on Windows, `$PORTLY_CONFIG` override): interval, ignore_ports, labels, log_files, [docker], [health]; themes dark/light/nord (config.rs) |
...
| 8 | Headless / non-TTY behavior for scripting & CI | lerd exits non-zero when stdout isn't a TTY; Homedash --test-mode | `portly --once` prints snapshot table and exits; doubles as bench harness input; `--once --json` emits one stable machine-readable snapshot (schema in README §Scripting, golden-tested field names) — bare `--json` refuses pre-TTY like the non-TTY guard; field-aware query tokens stay deferred (row 9) |
