...
| 5 | TOML config + themes, zero-config default preserved | Homedash YAML w/ unknown-field rejection; statui layered TOML; corsa themes | `~/.config/portly/config.toml` (`%APPDATA%\portly\config.toml` on Windows, `$PORTLY_CONFIG` override): interval, ignore_ports, labels, log_files, [docker], [health]; themes dark/light/nord (config.rs) |
